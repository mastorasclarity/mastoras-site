# Mástoras Maximum Impact Implementation Plan

## IMMEDIATE ACTIONS (This Week)

### ✅ Phase 1: Technical Foundation (Days 1-2)

#### 1.1 Google Search Console Setup
- [x] Go to [search.google.com/search-console](https://search.google.com/search-console)
- [x] Add property: `https://mastoras.netlify.app`
- [x] Verify with HTML meta tag (we have it in index.html)
- [x] Submit sitemap: `sitemap.xml`
- [ ] Request indexing for homepage — waiting for data to process (Google advised ~3-7 days)
- [ ] Request indexing for blog posts — same, waiting
- [ ] Monitor indexing status daily

#### 1.2 Bing Webmaster Tools Setup
- [x] Go to [bing.com/webmasters](https://www.bing.com/webmasters)
- [x] Add site: `https://mastoras.netlify.app`
- [x] Verify (imported from GSC)
- [x] Submit sitemap
- [x] Set geographic target to United Kingdom
- [ ] Monitor crawl status

#### 1.3 Apple Maps Connect Setup
- [ ] Go to [mapsconnect.apple.com](https://mapsconnect.apple.com/)
- [ ] Add business location
- [ ] Verify ownership
- [ ] Add photos and service descriptions

---

### ✅ Phase 2: Local Business Listings (Days 3-5)

#### 2.1 Google My Business Setup
**CRITICAL - Do First!**
- [x] Go to [business.google.com](https://www.google.com/business/)
- [x] Create listing for Mástoras
- [x] Service area: Causeway Coast and Glens (hidden address)
- [x] Phone: 07842 115153
- [x] Website: `https://mastoras.netlify.app`
- [x] Business category: Business Consultant
- [x] Business description added
- [x] Logo uploaded as profile photo
- [ ] Verification pending — Google processing (up to 5 days) ⏳ submitted 2026-03-26
- [ ] Once verified: add 5+ high-quality photos
- [ ] Once verified: set opening hours (Mon-Fri 9AM-5PM)
- [ ] Once verified: add business posts with links to blog

#### 2.2 Bing Places for Business
- [ ] Go to [bingplaces.com](https://www.bingplaces.com/)
- [ ] Add business listing (same info as GMB)
- [ ] Link to Google My Business for easy sync
- [ ] Add photos and videos
- [ ] Enable customer messaging

---

### ✅ Phase 3: Social Media Foundation (Days 4-7)

#### 3.1 LinkedIn Company Page
**HIGHEST PRIORITY**
- [ ] Go to [linkedin.com/company/new](https://www.linkedin.com/company/new/)
- [ ] Create company page: "Mástoras"
- [ ] Add profile photo (logo)
- [ ] Add cover photo (professional branding)
- [ ] Add company description:
  ```
  Clarity & Story Consultancy for Northern Irish Businesses
  
  We help founders, trades, and community organisations cut through the noise with:
  • Honest feasibility checks
  • Compelling brand stories
  • Strategic grant writing
  • Business clarity consulting
  
  Structure → Story → Soul
  ```
- [ ] Add website: `https://mastoras.netlify.app`
- [ ] Add industry: Management Consulting
- [ ] Add company size: 1-10 employees
- [ ] Set headquarters: Causeway Coast and Glens, Northern Ireland
- [ ] Verify page
- [ ] Add first posts immediately

#### 3.2 Twitter/X Business Account
- [ ] Create business account if not already done
- [ ] Username: @mastoras_ni (or available variant)
- [ ] Bio: "Clarity & Story Consultancy | Northern Ireland | Structure → Story → Soul"
- [ ] Add website link
- [ ] Add location: Northern Ireland
- [ ] Add profile picture (logo)
- [ ] Add header image (professional business imagery)
- [ ] Link to website in profile

#### 3.3 Facebook Business Page
- [ ] Go to [facebook.com/pages/create](https://www.facebook.com/pages/create/)
- [ ] Create business page: "Mástoras Consultancy"
- [ ] Category: Business Consultant
- [ ] Add description
- [ ] Add profile picture and cover photo
- [ ] Add website link
- [ ] Add business hours
- [ ] Add contact information

#### 3.4 Instagram Business Account
- [ ] Create business account or convert personal account
- [ ] Username: @mastoras.consultancy
- [ ] Bio: "Clarity & Story Consultancy | Structure → Story → Soul"
- [ ] Add professional photo
- [ ] Add website link
- [ ] Add contact button
- [ ] Create first 5 posts/reels

---

## SHORT-TERM ACTIONS (Next Month)

### ✅ Phase 4: Content Publishing (Week 2-3)

#### 4.1 Blog Publishing Schedule
- [ ] Publish one blog post per week
- [ ] Use Structure → Story → Soul article as template
- [ ] Topics:
  - [ ] Week 2: "Grant Writing for Northern Ireland Organizations"
  - [ ] Week 3: "How to Pitch Your Business to Investors"
  - [ ] Week 4: "Local Success: Causeway Coast Businesses That Triumphed"
- [ ] Optimize each post with:
  - [ ] AI-optimized title and meta descriptions
  - [ ] Schema.org structured data
  - [ ] Internal linking to other blog posts
  - [ ] Call-to-action to book consultation

#### 4.2 Content Distribution
- [ ] Share blog posts on all social platforms
- [ ] Create social media snippets (quotes, tips)
- [ ] Update RSS feed after each post
- [ ] Request indexing in Search Console

---

### ✅ Phase 5: Social Media Activation (Week 2-4)

#### 5.1 LinkedIn Content Strategy
**Post 3x per week using templates:**
- [ ] Monday: Business insight post
- [ ] Wednesday: Case study or tip
- [ ] Friday: Industry news + engagement

**Engagement Activities:**
- [ ] Comment on 5 industry posts daily
- [ ] Share relevant Northern Ireland business content
- [ ] Join LinkedIn groups (Business Consultants, NI Business)
- [ ] Send connection requests to local business owners

#### 5.2 Twitter/X Content Strategy
**Post 4-5 times per week:**
- [ ] Daily business tips
- [ ] Share blog content
- [ ] Engage with NI business accounts
- [ ] Thread format for insights

#### 5.3 Facebook Content Strategy
**Post 3x per week:**
- [ ] Share blog content
- [ ] Local business news
- [ ] Community engagement
- [ ] Ask questions to drive engagement

#### 5.4 Instagram Content Strategy
**Post 3-4x per week:**
- [ ] Share insights as graphics/quotes
- [ ] Post 2+ Reels per week (business tips)
- [ ] Stories daily (behind scenes, tips)
- [ ] Professional imagery focus

---

### ✅ Phase 6: Local Networking (Week 2-4)

#### 6.1 Join Business Networks
- [ ] **Northern Ireland Chamber of Commerce**
  - URL: northernirelandchamber.com
  - Attend monthly meetings
  - Join online groups

- [ ] **Federation of Small Businesses (FSB)**
  - URL: fsbni.org
  - Join local group
  - Attend networking events

- [ ] **Institute of Directors (IoD)**
  - URL: iodni.com
  - Research membership
  - Attend events

#### 6.2 Event Calendar
- [ ] Identify 2-3 upcoming local business events
- [ ] Register and attend
- [ ] Network and gather contacts
- [ ] Take photos for social media
- [ ] Write blog post about insights

#### 6.3 Local Directory Submissions (Batch Process)
**Start with Priority 1 (this week):**
- [ ] Google My Business ✅ (already doing)
- [ ] Bing Places ✅ (already doing)
- [ ] Apple Maps ✅ (already doing)

**Week 3: Priority 2:**
- [ ] NI Business Info - [nibusinessinfo.co.uk](https://www.nibusinessinfo.co.uk/)
- [ ] Derry Chamber of Commerce - [derrychamber.com](https://www.derrychamber.com/)
- [ ] Causeway Coast & Glens Council - [causewaycoastandglens.gov.uk](https://www.causewaycoastandglens.gov.uk/)

**Week 4: Priority 3:**
- [ ] Yell.com - [yell.com](https://www.yell.com/)
- [ ] Thomson Local - [thomsonlocal.com](https://www.thomsonlocal.com/)
- [ ] Touch Local - [touchlocal.com](https://www.touchlocal.com/)

---

## ONGOING ACTIONS (Monthly & Beyond)

### ✅ Phase 7: Monitoring & Analytics

#### 7.1 Weekly Monitoring Checklist
- [ ] **Google Search Console**
  - Review clicks and impressions
  - Check indexing status
  - Monitor crawl errors
  - Submit new content

- [ ] **Bing Webmaster Tools**
  - Check crawl health
  - Review keywords
  - Monitor mobile usability

- [ ] **Google Analytics**
  - Traffic sources
  - Top pages
  - Conversion tracking
  - Bounce rate analysis

- [ ] **Social Media Analytics**
  - Engagement rates
  - Follower growth
  - Top performing posts
  - Click-through rates

#### 7.2 Monthly Deep Dive
- [ ] Comprehensive traffic analysis
- [ ] Compare performance across platforms
- [ ] Review and adjust strategy
- [ ] Plan next month's content calendar
- [ ] Check citation accuracy across directories
- [ ] Respond to reviews and comments

#### 7.3 Quarterly Review
- [ ] Analyze overall growth metrics
- [ ] ROI calculation
- [ ] Competitive analysis
- [ ] Strategy adjustments
- [ ] Content pillar analysis
- [ ] New opportunity identification

---

### ✅ Phase 8: Ongoing Content Creation

#### 8.1 Blog Posting Schedule
**Target: 1 post per week minimum**

Monthly Content Themes:
- **Week 1:** Business Strategy insights
- **Week 2:** Grant Writing & Funding tips
- **Week 3:** Brand Storytelling techniques
- **Week 4:** Case studies & testimonials

#### 8.2 Content Repurposing
- [ ] Blog post → 5 social media posts
- [ ] Long-form article → LinkedIn article
- [ ] Case study → Multiple platform stories
- [ ] Webinar → Blog post series

#### 8.3 Guest Content Opportunities
- [ ] Contribute to industry publications
- [ ] Guest blog on complementary sites
- [ ] Contribute to local business blogs
- [ ] Interview opportunities

---

### ✅ Phase 9: Link Building & Authority

#### 9.1 Backlink Opportunities
- [ ] Get listed in industry directories
- [ ] Partner with local businesses
- [ ] Contribute to roundup articles
- [ ] Local media mentions
- [ ] Industry association listings

#### 9.2 Strategic Partnerships
- [ ] Identify complementary service providers
- [ ] Create cross-promotion opportunities
- [ ] Joint content creation
- [ ] Co-hosted webinars
- [ ] Referral partnerships

---

## CRITICAL SUCCESS METRICS

### Track Monthly
- **Website Traffic:** Target +20% monthly growth
- **Search Rankings:** Top 10 for "business consultancy Northern Ireland"
- **Social Followers:** Target 500+ LinkedIn followers by month 3
- **Blog Traffic:** 500+ monthly visitors by month 3
- **Conversion Rate:** Target 5%+ booking rate

### Red Flags to Monitor
- Sudden traffic drops (check indexing)
- Negative reviews (respond immediately)
- Competitor activity increases
- Algorithm changes affecting rankings

---

## BUDGET & TOOLS (Optional but Recommended)

### Free Tools
- Google Search Console
- Bing Webmaster Tools
- Google Analytics
- Social media platforms
- Canva (graphics)

### Paid Tools (Optional)
- **SEMrush** - Competitive analysis ($99/month)
- **Ahrefs** - Backlink research ($99/month)
- **Buffer/Later** - Social scheduling ($10-50/month)
- **Mailchimp** - Email marketing (free-$20/month)

---

## IMPLEMENTATION PRIORITY

### If Limited on Time, Focus On:
1. **Google My Business** - 30 minutes (HIGHEST ROI)
2. **LinkedIn** - 1 hour (HIGHEST ENGAGEMENT)
3. **Blog post** - 1-2 hours weekly (HIGHEST AUTHORITY)
4. **Social sharing** - 15 minutes daily (HIGHEST VISIBILITY)

---

## ACCOUNTABILITY CHECKLIST

### Week 1 Must-Haves
- [x] Google Search Console configured ✅ 2026-03-26
- [x] Bing Webmaster Tools configured ✅ 2026-03-26
- [ ] Google My Business verified
- [ ] LinkedIn company page created
- [ ] Twitter/Facebook/Instagram accounts optimized

### Month 1 Must-Haves
- [ ] 4 blog posts published
- [ ] 40+ social media posts
- [ ] 3 local directories updated
- [ ] 1 local business event attended
- [ ] 50+ new LinkedIn connections

### Month 3 Must-Haves
- [ ] 12+ blog posts
- [ ] 200+ social media posts
- [ ] Visible SEO improvements
- [ ] 500+ social media followers
- [ ] Monthly website revenue/leads established

---

## SUPPORT & RESOURCES

### Guides Available in Repository
- `/mastoras/analytics/google-search-console-setup.md`
- `/mastoras/analytics/bing-webmaster-tools-setup.md`
- `/mastoras/local-seo/business-directories-guide.md`
- `/mastoras/social/strategy.md`

### Quick Links
- Google Search Console: [search.google.com/search-console](https://search.google.com/search-console)
- Google My Business: [business.google.com](https://www.google.com/business/)
- Bing Webmaster: [bing.com/webmasters](https://www.bing.com/webmasters)
- LinkedIn: [linkedin.com](https://www.linkedin.com)

---

## Questions?

For detailed setup instructions for any tool, refer to the guides in your `/mastoras/analytics/` and `/mastoras/local-seo/` folders.

**Current Status:** Ready for Week 1 implementation
**Next Review Date:** 1 week from today
**Target Completion:** 30-45 days for all immediate + short-term actions